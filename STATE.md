...
**Last updated:** 2026-07-10 (Fri) — delivery, maintenance, and export code complete, pending live verification
...
- 🟨 NEXT: (a) native Iceberg streaming sinks plus listener-only metrics — **code-complete, pending live verification**; (b) Iceberg snapshot expiry, orphan cleanup, and compaction batch task — **code-complete, pending live verification**; (c) chaos demo recording (kill wiki-spark mid-batch, restart, show counts/aggregates match an uninterrupted control); (d) export gold Parquet and raw-event JSON into `exports/` — **code-complete, pending live verification**. After live verification: record the chaos demo and copy `exports/` to `portfolio-site`.
